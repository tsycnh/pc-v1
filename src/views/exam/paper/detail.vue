        ...
        <div class="records">
          ...
          <template v-else-if="joinRecords.length > 0">
            <div class="record-item" v-for="item in joinRecords" :key="item.id">
              <div class="item-type">
                <span v-if="item.status === 2">{{ item.score }}分</span>
                <span v-else-if="item.status === 3">已完成</span>
                <span v-else class="red">未完成</span>
              </div>
              <div class="item-pro">
                <span v-if="item.status === 1">考试中</span>
                <span v-else-if="item.status === 3">阅卷中</span>
                <span v-else-if="item.status === 2">已结束</span>
              </div>
              <div class="item-status" @click="join(item)">
                <span class="red" v-if="item.status === 1">继续考试</span>
                <span v-else>考试详情</span>
              </div>
            </div>
          </template>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.loading = true;
      this.$api.Exam.PaperDetail(this.id).then((res) => {
        this.loading = false;
